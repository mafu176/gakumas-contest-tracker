...
function isIpadArithmeticDebugEnabled() {
...
function combineIpadProductionRecoveries(tierCProductionRecovery, strictTotalProductionRecovery) {
  const tierCAppliedCases = Array.isArray(tierCProductionRecovery?.appliedCases)
    ? tierCProductionRecovery.appliedCases
    : [];
  const strictTotalAppliedCases = Array.isArray(strictTotalProductionRecovery?.appliedCases)
    ? strictTotalProductionRecovery.appliedCases
    : [];
  return {
    ...
    deviceMode:
      ...
      "ipad",
    appliedCases: [...tierCAppliedCases, ...strictTotalAppliedCases],
    subRecoveries: {
...
function ipadArithmeticProfilePreset(profile) {
  if (profile.kind === "invert-normalize") return "ipad-invert-normalize";
  if (profile.kind === "white-mask") return "ipad-white-mask";
  if (profile.kind === "blue-bonus-mask") return "ipad-blue-bonus-mask";
  return undefined;
}

const ipadCurrentPrimaryPriorities = {
  member: ["baseline-score-preprocess-3x-psm7"],
  bonus: ["blue-bonus-mask-3x-psm7"],
  total: ["white-mask-3x-psm7"],
};

function ipadCandidateProfilePriority(candidate, priority = []) {
...
function selectIpadCurrentPrimaryCandidate(pool = {}) {
  const priority = ipadCurrentPrimaryPriorities[pool.fieldType] || [];
  const candidateList = (pool.candidates || []).filter(
    (candidate) => ipadCandidateProfilePriority(candidate, priority) === 0
  );
  if (!candidateList.length) {
    return { value: 0, candidate: null, reason: "empty-current-primary-profile" };
  }
  const scored = candidateList
    .map((candidate, index) => ({
      ...
      score:
        1000 +
        (pool.fieldType === "bonus" && candidate.confidenceSignals?.plusLike ? 20 : 0) -
        Number(candidate.contributions?.[0]?.candidateIndex || 0),
      tieBreak: index,
    }))
    .sort((a, b) => {
      if (b.score !== a.score) return b.score - a.score;
      if (a.candidate.sourceRank !== b.candidate.sourceRank) {
        return a.candidate.sourceRank - b.candidate.sourceRank;
      }
      if (a.tieBreak !== b.tieBreak) return a.tieBreak - b.tieBreak;
      return a.candidate.value - b.candidate.value;
    });
  return {
    value: scored[0].candidate.value,
...
async function buildIpadArithmeticBrowserDiagnostics({ image, imageName, filter = {} }) {
  ...
  const diagnostics = {
    ...
    debugFlag: "ipadArithmeticDebug=1",
  ...
  if (!detection.detected) {
    diagnostics.blockReason = "not-ipad-layout";
    return diagnostics;
  }

  const template = buildIpadArithmeticRoiTemplate(image);
  const profiles = getIpadArithmeticPreprocessingProfiles();
  ...
  for (const field of filteredFields) {
    ...
    for (const profile of applicableProfiles) {
      const zone = padIpadArithmeticFieldZone(field, image, profile.paddingRatio ?? 0.12);
    ...
    const key = ipadArithmeticFieldKey({
      imageName,
      stage: field.stage,
      side: field.side,
      field: field.field,
    ...
    diagnostics.fieldPools.push(pool);
  }
    ...
    for (const side of sidesToEvaluate) {
      ...
      const currentSelections = Object.fromEntries(
        Object.entries(fieldCandidatePools).map(([label, pool]) => [
          label,
          selectIpadCurrentPrimaryCandidate(pool),
        ])
      );
      const currentPrimary = {
        members: [
          currentSelections.member1.value,
          currentSelections.member2.value,
          currentSelections.member3.value,
        ],
        bonus: currentSelections.bonus.value,
        total: currentSelections.total.value,
      };
      const tierC = evaluateIpadArithmeticSideSelectionTier({
        deviceMode: "ipad",
        fieldCandidatePools,
        currentPrimary,
        tier: "tier-c",
      ...
      const sideDiagnostics = {
        ...
        stage,
        side,
        candidatePools: fieldCandidatePools,
        currentPrimary,
        currentSelections,
        tierC,
        strictTotalSelectionEvidence,
...
import {
  ...
  getIpadArithmeticProfilesForFieldType,
    ...
    try {
      ...
      if (isProductionIpadArithmeticLayout(ipadArithmeticEvidenceData)) {
        ...
        let finalIpadArithmeticDiagnostics = {
          ...ipadArithmeticEvidenceData,
          note:
            "Browser-native iPad arithmetic evidence. Tier C and strict-total proposals are applied only by narrow production iPad recoveries when strict guards pass.",
        ...
        const ipadStrictTotalProductionResult =
          applyIpadStrictTotalSelectionRecovery(
            correctedStageScores,
            finalIpadArithmeticDiagnostics.strictTotalSelectionEvidence
          );
        const finalStageScores = ipadStrictTotalProductionResult.stageScores;
        const combinedIpadProductionRecovery = combineIpadProductionRecoveries(
        ...
        const ipadStrictTotalProductionLogs =
          ipadStrictTotalProductionResult.productionRecovery.appliedCases.map(
            (application) =>
              `ipadStrictTotalSelectionRecovery applied recoveryId=${IPAD_STRICT_TOTAL_SELECTION_RECOVERY_ID} stage=${application.stage} side=${application.side} members=${application.newValues.members.join(",")} bonus=${application.newValues.bonus} previousTotal=${application.oldValues.total} correctedTotal=${application.newValues.total} observedTotalProvenance=${application.provenance?.observedTotal?.profileIds?.join("+") || "observed"} candidateCount=${application.observedTotalCandidateCount} uniqueMatch=${application.uniqueMatchingObservedTotal}`
          );
        setOcrText(
          [
      ...
      for (const stage of stages) {
        ...
        const dropNoiseThirdMemberWhenPartialBonusMatchesTotal = (
          ...
          const totalCrownBonusRecovery = applySmartphoneTotalCrownBonusRecovery(
            ...
            bonusCandidates,
            rawCandidates,
            { mode: activeOcrMode, stage }
          );
          if (totalCrownBonusRecovery.applied) {
            correctionLogs.push(
              `totalCrownBonusRecovery kept members=${totalCrownBonusRecovery.members.join(",")} total=${totalCrownBonusRecovery.total} bonus=${totalCrownBonusRecovery.bonus}`
            );
            return {
        ...
        if (currentPcGroupedRawEvidence) {
          ...
          const currentPcStageWideSixMemberCandidateSolverRecovery =
            applyCurrentPcStageWideSixMemberCandidateSolverRecovery({
              stage,
              selectedSelfMembers: correctedSelfMembers,
              selectedEnemyMembers: correctedEnemyMembers,
          ...
          if (currentPcStageWideSixMemberCandidateSolverRecovery.applied) {
            correctionLogs.push(currentPcStageWideSixMemberCandidateSolverRecovery.message);
            correctedSelfMembers =
              currentPcStageWideSixMemberCandidateSolverRecovery.self.members;
            correctedEnemyMembers =
              currentPcStageWideSixMemberCandidateSolverRecovery.enemy.members;
          ...
          const currentPcExactMembersCrownBonusTotalRecovery = {
            self: applyCurrentPcExactMembersCrownBonusTotalRecovery({
              stage,
              side: "self",
              selectedMembers: correctedSelfMembers,
              selectedTotal: selfTotal,
              simulation: currentPcExactMembersCrownBonusTotalRecoverySimulation.self,
              layoutDetection: currentPcLayoutDetection,
              mode: "current-pc",
            }),
            enemy: applyCurrentPcExactMembersCrownBonusTotalRecovery({
              stage,
              side: "enemy",
              selectedMembers: correctedEnemyMembers,
              selectedTotal: enemyTotal,
              simulation: currentPcExactMembersCrownBonusTotalRecoverySimulation.enemy,
              layoutDetection: currentPcLayoutDetection,
              mode: "current-pc",
            }),
          };
          if (currentPcExactMembersCrownBonusTotalRecovery.self.applied) {
            correctionLogs.push(currentPcExactMembersCrownBonusTotalRecovery.self.message);
            correctedSelfMembers = currentPcExactMembersCrownBonusTotalRecovery.self.members;
            selfTotal = currentPcExactMembersCrownBonusTotalRecovery.self.total;
          }
          if (currentPcExactMembersCrownBonusTotalRecovery.enemy.applied) {
            correctionLogs.push(currentPcExactMembersCrownBonusTotalRecovery.enemy.message);
      ...
      finalIpadArithmeticDiagnostics = finalIpadArithmeticDiagnostics
        ...
        : null;
      const ipadStrictTotalProductionResult = finalIpadArithmeticDiagnostics
        ? applyIpadStrictTotalSelectionRecovery(
            correctedStageScores,
            finalIpadArithmeticDiagnostics.strictTotalSelectionEvidence
          )
        : { stageScores: correctedStageScores, productionRecovery: null };
      const finalStageScores = ipadStrictTotalProductionResult.stageScores;
      const combinedIpadProductionRecovery = combineIpadProductionRecoveries(
        ipadTierCProductionResult.productionRecovery,
        ipadStrictTotalProductionResult.productionRecovery
