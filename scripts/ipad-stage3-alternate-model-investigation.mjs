...
const artifactDir = path.join(rootDir, "tmp", "ipad-stage3-alternate-model");
...
function fieldExactSummary(candidateRows, expectedLookup) {
  ...
  for (const field of fields) {
    summary[field] = { exact: 0, wrongCandidateFields: 0, empty: 0, total: 76 };
  }
...
async function main() {
  ...
  if (
    baseline.fixtureCount !== 38 ||
    baselineRun.imagesProcessed !== 38 ||
    baselineRun.stageSidePass !== 112 ||
    baselineRun.tp !== 81 ||
    baselineRun.fp !== 0
  ) {
    throw new Error(`Expanded baseline mismatch; stop before alternate OCR investigation: ${JSON.stringify(baselineRun)}`);
  }
  ...
  const stressTests = {
    attempted: 0,
    reason:
      "No candidate architecture met the minimum member2/member3 threshold, so unlabeled stress testing was skipped to avoid spending runtime on a rejected direction.",
  };
  const stability = {
    attemptedRuns: 1,
    secondRunAttempted: false,
    reason:
      "RapidOCR did not meet viability thresholds in the first full run; two-run stability is reserved for the best viable candidate.",
  };
  const scorecard = {
    currentTesseract: tesseractStage3Baseline,
    rapidOcr: {
  ...
  const recommendation = {
    ...
    alternateEngineIntegrationJustified:
      simulationScore.tpStageSides >= 8 && simulationScore.fpStageSides === 0,
    recommendedNextStep:
      simulationScore.tpStageSides >= 8 && simulationScore.fpStageSides === 0
        ? "Run browser-model architecture parity for the winning candidate."
        : fieldSummary.member2.exact >= 10 || fieldSummary.member3.exact >= 10
          ? "Recognition headroom exists, but do not integrate yet. Investigate a browser-deployable model path and add stricter FP controls before parity."
          : "Do not integrate RapidOCR. Pause Stage3 OCR engine work or investigate a custom digit model only after finishing/labeling the broader iPad fixture set.",
    reason:
      simulationScore.fpStageSides > 0
        ? "RapidOCR clears the member2/member3 recognition threshold, but the strict selector simulation produced an FP."
        : "The tested alternate engine did not reach the required Stage3 simulation gain.",
  };
  const summary = {
    schema: "ipad-stage3-alternate-model-investigation-summary-v1",
    productionBaseline: {
    ...
    selectorSimulation: simulationScore,
    upperBounds,
    runtime,
    stability,
    stressTests,
  ...
  await writeJson("selector-simulation.json", simulationScore);
  ...
  await writeJson("stress-tests.json", stressTests);
