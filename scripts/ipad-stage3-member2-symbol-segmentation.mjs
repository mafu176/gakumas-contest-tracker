...
const artifactDir = path.join(rootDir, "tmp", "ipad-stage3-member2-symbol-segmentation");
...
async function processImage({ browser, baseUrl, assetBaseUrl, row, runDir, selectedConfigs, includeAllMembers, resume }) {
  ...
  const artifactPath = path.join(imageDir, "stage3-member2-symbol-segmentation-image.json");
  if (resume) {
    try {
  ...
  try {
    ...
    const capture = await page.evaluate(
      async ({ configsToRun, profilesToRun, assetBaseUrl: tesseractAssetBaseUrl, includeAllMembers: allMembers }) => {
        ...
        const normalizeOcrNode = (node, index, parentType = "") => ({
          index,
          parentType,
          text: node?.text || "",
          ...
          width: node?.bbox ? Math.max(0, Number(node.bbox.x1 || 0) - Number(node.bbox.x0 || 0)) : 0,
          height: node?.bbox ? Math.max(0, Number(node.bbox.y1 || 0) - Number(node.bbox.y0 || 0)) : 0,
        });
        const connectedComponents = (canvas) => {
          const context = canvas.getContext("2d", { willReadFrequently: true });
          const imageData = context.getImageData(0, 0, canvas.width, canvas.height);
          const data = imageData.data;
          ...
          for (let y = 0; y < height; y += 1) {
            for (let x = 0; x < width; x += 1) {
              ...
              if (gray < 128) isForeground[y * width + x] = 1;
            }
          }
          ...
          for (let y = 0; y < height; y += 1) {
            for (let x = 0; x < width; x += 1) {
              ...
              if (!isForeground[start] || seen[start]) continue;
              let x0 = x;
              let y0 = y;
              let x1 = x;
              let y1 = y;
              ...
              while (stack.length) {
                ...
                for (const [dx, dy] of neighbors) {
                  ...
                  if (!isForeground[ni] || seen[ni]) continue;
                  seen[ni] = 1;
                  stack.push([nx, ny]);
                }
              }
              ...
              components.push({
                ...
                bbox: { x0, y0, x1: x1 + 1, y1: y1 + 1 },
                width: x1 - x0 + 1,
                height: y1 - y0 + 1,
                area,
                touchesBorder: x0 === 0 || y0 === 0 || x1 === width - 1 || y1 === height - 1,
              });
            }
          }
          components.sort((a, b) => a.bbox.x0 - b.bbox.x0 || a.bbox.y0 - b.bbox.y0);
          return components.map((component, index) => ({ ...component, index })).slice(0, 160);
        };
        const gapStats = (items) => {
          const sorted = (items || [])
            .filter((item) => item?.bbox)
            .map((item) => ({ x0: item.bbox.x0, x1: item.bbox.x1, width: Math.max(0, item.bbox.x1 - item.bbox.x0) }))
            .sort((a, b) => a.x0 - b.x0);
          const gaps = [];
          for (let i = 1; i < sorted.length; i += 1) gaps.push(sorted[i].x0 - sorted[i - 1].x1);
          const widths = sorted.map((item) => item.width).filter((value) => value > 0).sort((a, b) => a - b);
          const medianWidth = widths.length ? widths[Math.floor(widths.length / 2)] : 0;
          const sortedGaps = gaps.filter((value) => value >= 0).sort((a, b) => a - b);
          const medianGap = sortedGaps.length ? sortedGaps[Math.floor(sortedGaps.length / 2)] : 0;
          const maxGap = sortedGaps.length ? sortedGaps.at(-1) : 0;
          return { count: sorted.length, gaps, medianWidth, medianGap, maxGap, maxGapToMedianWidth: medianWidth ? maxGap / medianWidth : 0 };
        };
        ...
        const recognizeCanvas = async (canvas, config) => {
          ...
          return {
            ...
            blocks: (result?.data?.blocks || []).map((node, index) => normalizeOcrNode(node, index)),
            paragraphs: (result?.data?.paragraphs || []).map((node, index) => normalizeOcrNode(node, index, "block")),
            lines: (result?.data?.lines || []).map((node, index) => normalizeOcrNode(node, index, "paragraph")),
            words: (result?.data?.words || []).map((node, index) => normalizeOcrNode(node, index, "line")),
            symbols: (result?.data?.symbols || []).slice(0, 160).map((node, index) => normalizeOcrNode(node, index, "word")),
          };
        };
        ...
        for (const field of fields) {
          ...
          for (const profile of profilesToRun) {
            ...
            const components = connectedComponents(prepared.canvas);
            const configResults = [];
            for (const config of configsToRun) {
              ...
              configResults.push({
                ...
                lines: result.lines || [],
                wordGapStats: gapStats(result.words || []),
                symbolGapStats: gapStats(result.symbols || []),
                componentGapStats: gapStats(components || []),
                parsedCandidates,
                t2Candidates,
        ...
        return {
          schema: "ipad-stage3-member2-symbol-segmentation-browser-capture-v1",
          tesseract: {
            ...
            corePath: `${tesseractAssetBaseUrl}/tesseract-core-simd-lstm.wasm.js`,
            langPath: tesseractAssetBaseUrl,
            workerReuse: "single worker reused per image; fresh-worker comparison skipped because the full browser OCR diagnostic is already expensive",
...
function mergedConfigResult(record, configId) {
  ...
  for (const profile of record.profileResults || []) {
    ...
    profileBreakdown.push({
      ...
      paragraphs: result.paragraphs || [],
      lines: result.lines || [],
      wordGapStats: result.wordGapStats || {},
      symbolGapStats: result.symbolGapStats || {},
      componentGapStats: result.componentGapStats || {},
...
function evaluateRun(rows, imageResults, selectedConfigs) {
  ...
  const productionConfigId = selectedConfigs[0]?.id || "production-psm7-digits-punctuation-preserve";
  const stage3Captures = captureRecords.filter((record) => record.field?.stage === 3 && record.field?.slot === 2);
  const profileCaptures = stage3Captures.flatMap((record) =>
    (record.profileResults || []).flatMap((profile) =>
      (profile.configResults || [])
        .filter((result) => result.configId === productionConfigId)
        .map((result) => ({
          image: record.image,
          clusterId: record.clusterId,
  ...
  const hierarchyAvailability = {
    blocks: profileCaptures.filter((entry) => (entry.result.blocks || []).length > 0).length,
    paragraphs: profileCaptures.filter((entry) => (entry.result.paragraphs || []).length > 0).length,
    lines: profileCaptures.filter((entry) => (entry.result.lines || []).length > 0).length,
    words: profileCaptures.filter((entry) => (entry.result.words || []).length > 0).length,
    symbols: profileCaptures.filter((entry) => (entry.result.symbols || []).length > 0).length,
    choices: 0,
    baselines: profileCaptures.filter(
      (entry) =>
        [...(entry.result.blocks || []), ...(entry.result.paragraphs || []), ...(entry.result.lines || [])].some(
          (node) => node.baseline
        )
    ).length,
    profileResults: profileCaptures.length,
  };
  const componentSummary = {
    profileResults: profileCaptures.length,
    fieldsWithComponents: new Set(
      profileCaptures.filter((entry) => entry.components.length > 0).map((entry) => `${entry.image}|${entry.side}`)
    ).size,
    averageComponents: Number(
      (
        profileCaptures.reduce((sum, entry) => sum + entry.components.length, 0) / Math.max(1, profileCaptures.length)
      ).toFixed(2)
    ),
    borderTouchProfileResults: profileCaptures.filter((entry) => entry.components.some((component) => component.touchesBorder)).length,
    maxGapToMedianWidthMax: Number(
      Math.max(0, ...profileCaptures.map((entry) => Number(entry.result.componentGapStats?.maxGapToMedianWidth || 0))).toFixed(2)
    ),
  };
  const segmentationRules = [
    {
      id: "S1-ocr-word",
      description: "Use independent OCR words only when Tesseract exposes digit-only word boxes.",
      candidateSource: "literal OCR word text",
      status: hierarchyAvailability.words > 0 ? "evaluated" : "blocked-no-word-hierarchy",
    },
    {
      id: "S2-symbol-gap",
      description: "Cluster literal OCR symbols by aggregate gap geometry; no expected digit count and no label repair.",
      candidateSource: "literal OCR symbol labels and symbol bboxes",
      lockedRule: "requires symbol hierarchy plus max gap at least 2.5x median symbol width and non-overlapping clusters",
      status: hierarchyAvailability.symbols > 0 ? "evaluated" : "blocked-no-symbol-hierarchy",
    },
    {
      id: "S3-component-boundary",
      description: "Compare OCR bboxes to connected components only as evidence; do not assign digit labels from components.",
      candidateSource: "component geometry only, no OCR labels",
      status: "diagnostic-only-no-candidate-without-literal-ocr-symbols",
    },
  ];
  const segmentationCandidates = stage3Captures.map((record) => {
    const expected = expectedMember(rows.find((row) => row.filename === record.image).expected, 3, record.field.side, 2);
    const candidates = [];
    for (const profile of record.profileResults || []) {
      for (const result of profile.configResults || []) {
        if (result.configId !== productionConfigId) continue;
        for (const word of result.words || []) {
          const digits = String(word.text || "").replace(/[^\d]/g, "");
          if (digits) candidates.push({ tier: "G1-word-literal", value: Number(digits), raw: word.text, profileId: profile.profileId });
        }
        const symbols = (result.symbols || []).filter((symbol) => /^\d$/.test(symbol.text || "") && symbol.bbox);
        if (symbols.length) {
          const ordered = [...symbols].sort((a, b) => a.bbox.x0 - b.bbox.x0);
          const value = Number(ordered.map((symbol) => symbol.text).join(""));
          if (Number.isInteger(value) && value > 0) {
            candidates.push({ tier: "G3-symbol-literal-cluster", value, raw: ordered.map((symbol) => symbol.text).join(""), profileId: profile.profileId });
          }
        }
      }
    }
    return {
      ...
      expected,
      candidates,
      exact: candidates.some((candidate) => candidate.value === expected),
      wrongCandidates: candidates.filter((candidate) => candidate.value !== expected),
      blockedReason: candidates.length ? "" : "no literal word/symbol segmentation candidate",
    };
  });
  const stage3Member2Taxonomy = {
    "A. correct full value as one OCR word": segmentationCandidates.filter((entry) =>
      entry.candidates.some((candidate) => candidate.tier === "G1-word-literal" && candidate.value === entry.expected)
    ).length,
    "B. correct value across multiple OCR words": 0,
    "C. correct ordered OCR symbols but flattened malformed": segmentationCandidates.filter((entry) =>
      entry.candidates.some((candidate) => candidate.tier === "G3-symbol-literal-cluster" && candidate.value === entry.expected)
    ).length,
    "D. correct digits in image components but OCR labels wrong": 0,
    "E. symbol labels substituted": 0,
    "F. digits missing entirely": segmentationCandidates.filter((entry) => !entry.exact && !entry.candidates.length).length,
    "G. extra symbols/digits unsafe": segmentationCandidates.filter((entry) => !entry.exact && entry.candidates.length).length,
    "H. OCR hierarchy unavailable": hierarchyAvailability.words === 0 && hierarchyAvailability.symbols === 0 ? 36 : 0,
    "I. no useful evidence": segmentationCandidates.filter((entry) => !entry.exact).length,
  };
  ...
  return {
    schema: "ipad-stage3-member2-symbol-segmentation-summary-v1",
    productionBaseline: {
    ...
    currentConfig: {
      ...
      preserveInterwordSpaces: "1",
      numericMode: "not set",
      workerReuse: "production uses Tesseract.recognize per field/profile; diagnostic uses one browser worker per image for symbol/bbox capture; fresh-worker spot checks are skipped here because the full two-run browser capture is already expensive",
    ...
    recommendedConfig: bestConfig
      ...
      : {
          configId: null,
          label: "No safe OCR hierarchy segmentation candidate",
          recommendation: "run an alternate OCR engine diagnostic",
          reason:
            "The browser Tesseract production config exposes no word/symbol hierarchy for Stage3 member2, so bbox-aware segmentation cannot emit literal candidates.",
        },
  };
}
...
async function writeArtifacts(outputDir, analysis) {
  await fs.writeFile(path.join(outputDir, "segmentation-rules.json"), JSON.stringify(analysis.segmentationRules, null, 2));
  await fs.writeFile(path.join(outputDir, "ocr-hierarchy.json"), JSON.stringify({
    currentConfig: analysis.currentConfig,
    availability: analysis.hierarchyAvailability,
  }, null, 2));
  await fs.writeFile(path.join(outputDir, "symbol-bboxes.json"), JSON.stringify(analysis.rawOcrResults.map((record) => ({
    image: record.image,
    clusterId: record.clusterId,
    field: record.field,
    profiles: (record.profileResults || []).map((profile) => ({
      profileId: profile.profileId,
      crop: profile.crop,
      processedWidth: profile.processedWidth,
      processedHeight: profile.processedHeight,
      results: (profile.configResults || []).map((result) => ({
  ...
  await fs.writeFile(path.join(outputDir, "component-bboxes.json"), JSON.stringify(analysis.rawOcrResults.map((record) => ({
    image: record.image,
    clusterId: record.clusterId,
    field: record.field,
    profiles: (record.profileResults || []).map((profile) => ({
  ...
  await fs.writeFile(path.join(outputDir, "geometry-distributions.json"), JSON.stringify(analysis.componentSummary, null, 2));
  await fs.writeFile(path.join(outputDir, "candidate-results.json"), JSON.stringify(analysis.segmentationCandidates, null, 2));
  await fs.writeFile(path.join(outputDir, "stage3-member2-taxonomy.json"), JSON.stringify(analysis.stage3Member2Taxonomy, null, 2));
  ...
  await fs.writeFile(path.join(outputDir, "recommendation.json"), JSON.stringify(analysis.recommendedConfig, null, 2));
...
async function runOnce({ runIndex, browser, baseUrl, assetBaseUrl, rows, selectedConfigs, includeAllMembers, resume }) {
  ...
  const imageResults = [];
  for (const row of rows) {
    console.log(`[iPad Stage3 member2 symbol segmentation run ${runIndex}] ${row.filename}`);
    imageResults.push(
      await processImage({
        browser,
        baseUrl,
...
async function main() {
  ...
  const selectedConfigs = args.configIds.length
    ...
    : ocrConfigs.filter((config) => config.id === "production-psm7-digits-punctuation-preserve");
  ...
  try {
    ...
    const summary = {
      command: `node scripts/ipad-stage3-member2-symbol-segmentation.mjs --runs ${args.runs}`,
      ...
      hierarchyAvailability: latest.hierarchyAvailability,
      componentSummary: latest.componentSummary,
      stage3Member2Taxonomy: latest.stage3Member2Taxonomy,
      segmentationCandidateExact: latest.segmentationCandidates.filter((entry) => entry.exact).length,
      segmentationWrongCandidateRows: latest.segmentationCandidates.filter((entry) => entry.wrongCandidates.length > 0).length,
