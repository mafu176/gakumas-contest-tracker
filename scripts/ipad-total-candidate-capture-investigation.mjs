...
const artifactDir = path.join(rootDir, "tmp", "ipad-total-candidate-capture");
...
const profiles = [
  { id: "production-white-mask-3x-psm7", label: "production-like white mask 3x PSM7", kind: "white-mask", scale: 3, pageSegMode: "7", threshold: 176 },
  { id: "white-mask-3x-psm7-digits-only", label: "white mask 3x PSM7 digits only", kind: "white-mask", scale: 3, pageSegMode: "7", threshold: 176, digitsOnly: true },
  ...
  return {
    ...
    baseUrl:
      baseUrlIndex >= 0
        ...
        : process.env.IPAD_TOTAL_CAPTURE_BASE_URL || "",
...
async function loadPlaywright() {
  try {
    ...
    throw new Error(
      [
        "Playwright is required for iPad browser total candidate capture.",
...
function poolFor(diagnostics, stage, side) {
  return (
    diagnostics?.stages?.[`stage${stage}`]?.[side]?.fieldCandidatePools?.total ||
    (diagnostics?.fieldPools || []).find(
      (pool) => pool.stage === stage && pool.side === side && pool.fieldType === "total"
    ) ||
    null
  );
}
...
function parseT1Standalone(text) {
  const values = [];
  for (const match of String(text || "").matchAll(/(?<![\d,.])\d{5,8}(?![\d,.])/g)) {
    values.push({ raw: match[0], value: normalizeTextNumber(match[0]), parser: "T1-standalone" });
  }
  return values.filter((entry) => entry.value >= 10000 && entry.value < 10000000);
}

function parseT2Grouped(text) {
  const values = [];
  for (const match of String(text || "").matchAll(/(?<!\d)\d{1,3}(?:[,.]\d{3})+(?!\d)/g)) {
    values.push({ raw: match[0], value: normalizeTextNumber(match[0]), parser: "T2-total-grouped" });
  }
  return values.filter((entry) => entry.value >= 10000 && entry.value < 10000000);
}
...
function parseT3NumericRuns(text) {
  const values = [];
  for (const match of String(text || "").matchAll(/(?<![A-Za-z0-9])[\d,. ]{5,14}(?![A-Za-z0-9])/g)) {
    const raw = match[0];
...
function classifyMissingTotal(record) {
  if (record.productionPresent) return "production-candidate-present";
  if (record.parserVariants.T2.some((entry) => entry.value === record.expected)) return "exact-total-recovered-by-grouped-parser";
  if (record.parserVariants.T1.some((entry) => entry.value === record.expected)) return "exact-total-recovered-by-standalone-parser";
  if (record.parserVariants.T3.some((entry) => entry.value === record.expected)) return "exact-total-recovered-by-numeric-run-parser";
  if (record.profileSummaries.some((profile) => profile.values.includes(record.expected.total))) {
    return "exact-total-recovered-by-profile-ocr";
  }
  const allRaw = [
    ...record.rawTexts.map((entry) => entry.rawText),
    ...record.profileSummaries.map((entry) => entry.rawText),
  ].join("\n");
  if (!allRaw.trim()) return "empty-total-ocr";
  if (allRaw.replace(/[^\d]/g, "").includes(String(record.expected))) return "exact-digits-present-but-not-tokenized";
  ...
  if (allValues.some((value) => Math.abs(value - record.expected) <= 9)) return "only-small-near-value-observed";
  if (allValues.some((value) => String(record.expected).includes(String(value)) || String(value).includes(String(record.expected)))) {
    return "fragment-or-concatenated-total-evidence";
  }
  return "exact-total-absent";
}
...
async function processImage({ browser, baseUrl, assetBaseUrl, row, runDir, selectedProfiles, resume }) {
  ...
  const artifactPath = path.join(imageDir, "total-capture-image.json");
  if (resume) {
    try {
  ...
  try {
    ...
    await page.evaluate((label) => {
      ...
      if (!file) throw new Error("No uploaded file available for iPad total candidate capture.");
    ...
    const capture = await page.evaluate(
      async ({ profilesToRun, assetBaseUrl: tesseractAssetBaseUrl }) => {
        ...
        const input = document.querySelector('[data-testid="ocr-screenshot-file-input"]');
        const file = input?.files?.[0];
        if (!file) throw new Error("No uploaded file available inside browser total capture.");
        ...
        const fields = (window.__IPAD_ARITHMETIC_DIAGNOSTICS__?.fieldPools || [])
          .filter((pool) => pool.fieldType === "total")
          .map((pool) => ({
            key: pool.key,
            stage: pool.stage,
            side: pool.side,
        ...
        return {
          schema: "ipad-total-candidate-capture-browser-ocr-v1",
...
function evaluate(rows, imageResults, selectedProfiles, productionBaseline = null) {
  ...
  for (const row of rows) {
    ...
    for (const stage of stages) {
      for (const side of sides) {
        ...
        const totalPool = poolFor(diagnostics, stage, side);
        const productionValuesForField = productionValues(totalPool);
        const productionPresent = productionValuesForField.includes(expected.total);
        const rawTexts = rawTextsFromPool(totalPool);
        ...
        const captureRecord = captureByKey.get(totalPool?.key || `${row.filename}|${stage}|${side}|total|0`);
        ...
        for (const profile of selectedProfiles) {
          ...
          const expectedPresent = values.includes(expected.total);
          const noiseValues = values.filter((value) => value !== expected.total && !productionValuesForField.includes(value));
          stats.fields += 1;
        ...
        record.taxonomy = classifyMissingTotal(record);
        totalFieldRecords.push(record);
        ...
        for (const profile of profileSummaries) {
          totalCandidateSets[`T0_${profile.profileId}`] = unique([...productionValuesForField, ...profile.values]);
        }
        const selectedMembersExact = finalComparison.membersPass;
        const selectedBonusExact = finalComparison.bonusPass;
        const addressableBy = Object.entries(totalCandidateSets)
          .filter(([, values]) => values.includes(expected.total))
          .map(([source]) => source);
        stageSideRecords.push({
          ...
          final: finalSide,
          addressableBy,
          totalMissingInProduction: !productionPresent,
          potentiallyAddressableByTotalOnly:
            !finalComparison.pass && selectedMembersExact && selectedBonusExact && !finalComparison.totalPass && addressableBy.length > 0,
        });
      }
    }
  }
  ...
  for (const record of totalFieldRecords.filter((entry) => !entry.productionPresent)) {
  ...
  const combinations = [];
  for (const key of ["T1", "T2", "T3"]) {
    combinations.push(evaluateCombination(totalFieldRecords, [`parser:${key}`]));
  }
  for (const profile of selectedProfiles) {
    combinations.push(evaluateCombination(totalFieldRecords, [`profile:${profile.id}`]));
  }
  combinations.push(evaluateCombination(totalFieldRecords, ["parser:T1", "parser:T2", "parser:T3"]));
  ...
  const addressableSides = stageSideRecords.filter((entry) => entry.potentiallyAddressableByTotalOnly);
  const tierCSimulation = {
    diagnosticsOnly: true,
    note: "This is not a recovery. It counts rows where final members and bonus are already exact, the final total is wrong, and exact total evidence appears in a parser/profile variant.",
  ...
  return {
    schema: "ipad-total-candidate-capture-investigation-summary-v1",
    totalImages: rows.length,
    totalFields: totalFieldRecords.length,
    productionBaseline,
    productionTotalCoverage: totalFieldRecords.filter((record) => record.productionPresent).length,
    productionTotalCoveragePct: pct(totalFieldRecords.filter((record) => record.productionPresent).length, totalFieldRecords.length),
    parserSummaries,
    profileSummaries,
  ...
  for (const record of records) {
    ...
    for (const source of sources) {
      ...
      for (const value of extraValues) {
        if (!record.productionValues.includes(value) && value !== record.expected.total) {
          hasNoise = true;
          noiseCandidates += 1;
        }
        values.add(value);
    ...
    if (values.has(record.expected.total)) observed += 1;
    if (!record.productionPresent && values.has(record.expected.total)) newExpectedFields += 1;
    if (hasNoise) noiseFields += 1;
  }
  return {
    sources,
    observedTotalFields: observed,
    newExpectedFields,
    noiseFields,
    noiseCandidates,
...
function recommend(profileSummaries, parserSummaries, tierCSimulation) {
  ...
  if (!best || best.newlyObservedExpected < 2 || best.noiseFieldPct > 20) {
    return {
      recommendation: "none",
      reason: "No parser/profile variant added at least two exact expected total candidates while keeping new noise low.",
      nextProductionExperiment: "Defer total capture production changes; use this artifact to target narrower crop/parser work.",
    };
  }
  return {
    recommendation: best.source,
    reason: `${best.source} adds ${best.newlyObservedExpected} exact expected totals with ${best.noiseFields} noisy fields. Tier-C total-only potential TP=${tierCSimulation.potentialTp}.`,
    nextProductionExperiment:
      "Runner/browser parity for a diagnostic-only total-evidence proposal would be justified only if the accepted rows have unchanged exact members and bonus.",
  };
}
...
async function runOnce({ runIndex, runDir, browser, baseUrl, assetBaseUrl, rows, selectedProfiles, resume }) {
  ...
  const imageResults = [];
  for (const row of rows) {
    console.log(`[iPad total candidate capture run ${runIndex}] ${row.filename}`);
  ...
  const pass =
    baseline.runs.every(
      (entry) =>
        ...
        entry.stageSidePass === 40 &&
        entry.productionApplications === 24 &&
        entry.tp === 24 &&
        entry.fp === 0
    ) &&
...
function compareRunStability(runSummaries) {
  ...
  const signature = (summary) =>
    JSON.stringify(
      summary.totalFieldRecords.map((record) => ({
        image: record.image,
        stage: record.stage,
        side: record.side,
...
async function writeTopLevelArtifacts(summary, runStability) {
  const failureTaxonomy = {
    totalRecognitionFailures: summary.totalFieldRecords.filter((record) => !record.productionPresent).length,
    taxonomyCounts: summary.taxonomyCounts,
    records: summary.totalFieldRecords
      .filter((record) => !record.productionPresent)
      .map(({ image, stage, side, expected, productionValues, taxonomy, parserValues, profileSummaries }) => ({
        image,
        stage,
        side,
  ...
  await writeJson(path.join(artifactDir, "total-field-audit.json"), summary.totalFieldRecords);
...
async function main() {
  ...
  const selectedProfiles = args.profileIds.length
    ...
    : profiles;
  if (!selectedProfiles.length) throw new Error(`No matching total profiles for: ${args.profileIds.join(",")}`);
  ...
  try {
    ...
    for (let runIndex = 1; runIndex <= args.runs; runIndex += 1) {
      ...
      await writeJson(path.join(runDir, "total-field-records.json"), summary.totalFieldRecords);
    ...
    const consoleSummary = {
      command: "node scripts/ipad-total-candidate-capture-investigation.mjs",
      artifactDir: rel(artifactDir),
      runs: args.runs,
      totalFields: latest.totalFields,
      productionTotalCoverage: latest.productionTotalCoverage,
      productionTotalCoveragePct: latest.productionTotalCoveragePct,
